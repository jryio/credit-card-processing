 ...
 * @return: null
 */
exports.charge = (name, amount, dataStore) => {
  dataStore.forEach((card) => {
    if (card.name === name &&
        card.isValidCardNum && !(card.balance + amount > card.limit)) {
      card.balance += amount
    }
  })
}
 ...
 * @return: null
 */
exports.credit = (name, amount, dataStore) => {
  dataStore.forEach((card) => {
    if (card.name === name && card.isValidCardNum) {
      card.balance -= amount
    }
  })
}
